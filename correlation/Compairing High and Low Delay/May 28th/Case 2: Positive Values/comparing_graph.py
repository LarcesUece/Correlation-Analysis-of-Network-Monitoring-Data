...
def graph (label, dict1, dict2, hours):
    ...
    plt.title(f'Delays for May 28th {label}', fontsize=18)
    plt.legend(bbox_to_anchor=(0.95, 1), fontsize=12)
    # plt.show()
    plt.savefig(f'28-05-2023 comparison {label}.pdf')
...
if __name__ == "__main__":
    ...
    df = pd.read_csv('soma dos positivos 28-05 ruim.csv')
    dflabel = df.columns.to_list()

    df2 = pd.read_csv('soma dos positivos 28-05 bom.csv')
